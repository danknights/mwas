...
run.heatmap <- function(otufile, mapfile)
{
	...
	result <- differentiation.test(otu, category=new.treatments)

	# make the heatmap
	# TODO: replace this with the parent plot function once it's ready
	mwas.heatmap(otu, map, cluster.var=c("Sex", "Treatment"), color.var=names(color.list), color.list, kegg_pathways=kegg_pathways, heatmap.title=heatmap.title, outputfile=outputfile)
}
